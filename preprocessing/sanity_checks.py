...
class InputDictionarySanityChecker:
    ...
    def _infer_assignment_dims(self, global_config) -> tuple[int | None, int | None] | None:
        event_info = getattr(global_config, "event_info", None)
        if event_info is None:
            return None

        event_particles = getattr(event_info, "event_particles", None)
        product_particles = getattr(event_info, "product_particles", None)
        if not event_particles or not product_particles:
            return None

        n_resonances = max(
            (len(getattr(particles, "names", []) or []) for particles in event_particles.values()),
            default=None,
        )

        max_children = None
        for process_particles in product_particles.values():
            if not isinstance(process_particles, dict):
                continue
            for particle in process_particles.values():
                names = getattr(particle, "names", None)
                if names is None:
                    continue
                count = len(names)
                max_children = count if max_children is None else max(max_children, count)

        if n_resonances is None and max_children is None:
            return None
        return n_resonances, max_children

    def _infer_segmentation_targets(self, global_config) -> tuple[int, int] | None:
        event_info = getattr(global_config, "event_info", None)
        if event_info is None:
            return None

        indices = getattr(event_info, "segmentation_indices", None)
        if indices is None:
            return None
        ...
        n_classes = n_targets + 1  # reserve a null/background class
        return n_targets, n_classes

    def _build_expectations(self, global_config) -> dict[str, ExpectedTensor]:
        expectations = {t.name: t for t in self.INPUT_TENSORS}
        for tensors in self.TASK_TENSORS.values():
            expectations.update({t.name: t for t in tensors})

        if global_config is None:
            return expectations

        assignment_dims = self._infer_assignment_dims(global_config)
        if assignment_dims is not None:
            n_resonances, max_children = assignment_dims
            if n_resonances is not None and max_children is not None:
                expectations["assignments-indices"] = ExpectedTensor(
                    "assignments-indices",
                    (None, n_resonances, max_children),
                    "Resonance-to-child mapping",
                )
                expectations["assignments-mask"] = ExpectedTensor(
                    "assignments-mask",
                    (None, n_resonances),
                    "Resonance validity mask",
                )
                expectations["assignments-indices-mask"] = ExpectedTensor(
                    "assignments-indices-mask",
                    (None, n_resonances, max_children),
                    "Per-child mask",
                )

        segmentation_targets = self._infer_segmentation_targets(global_config)
        if segmentation_targets is not None:
            n_targets, n_classes = segmentation_targets
            expectations["segmentation-class"] = ExpectedTensor(
                "segmentation-class",
                (None, n_targets, n_classes),
                "One-hot daughter class",
            )
            expectations["segmentation-full-class"] = ExpectedTensor(
                "segmentation-full-class",
                (None, n_targets, n_classes),
                "Complete-daughter indicator",
            )
            expectations["segmentation-data"] = ExpectedTensor(
                "segmentation-data",
                (None, n_targets, 18),
                "Assignment of daughter slots",
            )
            expectations["segmentation-momentum"] = ExpectedTensor(
                "segmentation-momentum",
                (None, n_targets, 4),
                "Daughter four-momenta",
            )
    ...
    def _validate_shapes(self, pdict: dict, n_events: int | None, expected_map: dict[str, ExpectedTensor]) -> list[list[str]]:
        rows: list[list[str]] = []
    ...
    def run(self, pdict: dict, *, global_config=None) -> None:
        ...
        expectations = self._build_expectations(global_config)
        ...
        validation_rows = self._validate_shapes(pdict, n_events, expectations)
